...
for (i in 1:10){
  ...
  em_rf <- caret::train(emergency_room_30~., data=data_train, method="rf", trControl = ctrl,metric="Kappa")
  #em_rf <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")
  #varImp(em_rf)
  
  
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(em_rf, newdata=data_test)
  ...
  em_rf %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-em_rf %>% test_roc(data = data_test) %>% auc()
...
tree_num <- which(em_rf$finalModel$forest$ndbigtree == median(em_rf$finalModel$forest$ndbigtree))[1]
tree_func(final_model = em_rf$finalModel, tree_num)

tree_func <- function(final_model =  em_rf$finalModel, 
                      tree_num) {
  
  # get tree by index
  tree <- randomForest::getTree(em_rf$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$emergency_room_30, predict(em_rf, data, type = "prob")[, "Yes"])
}
