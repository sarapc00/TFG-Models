...
data <- dataset[,!(names(dataset) %in% c("ID", "arr", 'iam1', "angor", "insufcardio", "shockhemodin", "aturadacardioresp", "shockhemoragic", "hemopostquir", "altrescardio", 
                                         ...
                                         "filter_.", "reintervention_30", "readmision_30"))]
...
for (i in 1:10){
  set.seed(73*i)
  h<-holdout(data$emergency_room_30,ratio=.70,mode="stratified")
  data_train <- data[h$tr,]
  data_test  <- data[h$ts,]
  
  print(table(data_train$emergency_room_30))
  print(table(data_test$emergency_room_30))
  ...
  em_dt <- caret::train(emergency_room_30~., data=data_train, method="rpart", trControl = ctrl,metric="Kappa")
  #em_dt <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")
  #varImp(em_dt)
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(em_dt, data_test)
  cm_under<-confusionMatrix(pred,data_test$emergency_room_30,positive="Yes")
  #draw_confusion_matrix(cm_under)
  test_roc <- function(model, data) {
    roc(data$emergency_room_30, predict(model, data, type = "prob")[, "Yes"])
  }
  em_dt %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-em_dt %>% test_roc(data = data_test) %>% auc()
  
  #tree_num <- which(em_dt$finalModel$forest$ndbigtree == median(em_dt$finalModel$forest$ndbigtree))[1]
  #tree_func(final_model = em_dt$finalModel, tree_num)
}
...
rpart.plot(em_dt$finalModel)

##??
tree_num <- which(em_dt$finalModel$forest$ndbigtree == median(em_dt$finalModel$forest$ndbigtree))[1]
tree_func(final_model = em_dt$finalModel, tree_num)

tree_func <- function(final_model =  em_dt$finalModel, 
                      tree_num) {
  
  
  
  # get tree by index
  tree <- randomForest::getTree(em_dt$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$emergency_room_30, predict(em_dt, data, type = "prob")[, "Yes"])
}
