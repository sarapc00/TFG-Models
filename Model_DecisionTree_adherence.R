...
for (i in 1:10){
  ...
  mod_rf <- caret::train(Completers~. , data=data_train, method="rpart", trControl = ctrl,metric="Kappa")
...
test_roc <- function(model, data) {
  roc(data$Completers, predict(mod_rf, data, type = "prob")[, "Completers"])
}
