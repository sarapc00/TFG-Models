...
data <- dataset[,!(names(dataset) %in% c("ID", "arr", 'iam1', "angor", "insufcardio", "shockhemodin", "aturadacardioresp", "shockhemoragic", "hemopostquir", "altrescardio", 
                                         ...
                                         "filter_.", "reintervention_30", "emergency_room_30"))]
...
for (i in 1:10){
  ...
  re_dt <- caret::train(readmision_30~., data=data_train, method="rpart", trControl = ctrl,metric="Kappa")
  #re_dt <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")
  #varImp(re_dt)
  
  
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(re_dt, data_test)
  cm_under<-confusionMatrix(pred,data_test$readmision_30,positive="Yes")
  #draw_confusion_matrix(cm_under)
  test_roc <- function(model, data) {
    roc(data$readmision_30, predict(model, data, type = "prob")[, "Yes"])
  }
  re_dt %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-re_dt %>% test_roc(data = data_test) %>% auc()
  
  #tree_num <- which(re_dt$finalModel$forest$ndbigtree == median(re_dt$finalModel$forest$ndbigtree))[1]
  #tree_func(final_model = re_dt$finalModel, tree_num)
}
...
rpart.plot(re_dt$finalModel)

##??
tree_num <- which(re_dt$finalModel$forest$ndbigtree == median(re_dt$finalModel$forest$ndbigtree))[1]
tree_func(final_model = re_dt$finalModel, tree_num)

tree_func <- function(final_model =  re_dt$finalModel, 
                      tree_num) {
  

  
  # get tree by index
  tree <- randomForest::getTree(re_dt$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$readmision_30, predict(re_dt, data, type = "prob")[, "Yes"])
}
