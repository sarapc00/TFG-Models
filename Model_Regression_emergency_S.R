...
for (i in 1:10){
  ...
  em_lr <- caret::train(emergency_room_30~., data=data_train, method="LogitBoost", trControl = ctrl,metric="Kappa")
  #em_lr <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")
  #varImp(em_lr)
  
  
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(em_lr, newdata=data_test)
  ...
  em_lr %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-em_lr %>% test_roc(data = data_test) %>% auc()
...
tree_num <- which(em_lr$finalModel$forest$ndbigtree == median(em_lr$finalModel$forest$ndbigtree))[1]
tree_func(final_model = em_lr$finalModel, tree_num)

tree_func <- function(final_model =  em_lr$finalModel, 
                      tree_num) {
  
  # get tree by index
  tree <- randomForest::getTree(em_lr$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$emergency_room_30, predict(em_lr, data, type = "prob")[, "Yes"])
}
