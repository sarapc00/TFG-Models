...
for (i in 1:10){
  ...
  re_lr <- caret::train(readmision_30~., data=data_train, method="LogitBoost", trControl = ctrl,metric="Kappa")
  #re_lr <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")
  #varImp(re_lr)

  
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(re_lr, data_test)
  ...
  re_lr %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-re_lr %>% test_roc(data = data_test) %>% auc()
  
  #tree_num <- which(re_lr$finalModel$forest$ndbigtree == median(re_lr$finalModel$forest$ndbigtree))[1]
  #tree_func(final_model = re_lr$finalModel, tree_num)
}
...
tree_func <- function(final_model =  re_lr$finalModel, 
                      tree_num) {
  
  # get tree by index
  tree <- randomForest::getTree(re_lr$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$readmision_30, predict(re_lr, data, type = "prob")[, "Yes"])
}
