...
for (i in 1:10){
  set.seed(73*8)
  h<-holdout(data$Completers,ratio=.70,mode="stratified")
  ...
  ad_rf <- caret::train(Completers~. , data=data_train, method="rf", trControl = ctrl,metric="Kappa")
  #ad_rf <- caret::train(readm_30~., data=data_train, method="rf", trControl = ctrl, preProc=c("center","scale"),metric="Kappa")

  
  # Test the new model on new and unseen Data for reproducibility
  pred = predict(ad_rf, data_test)
  ...
  ad_rf %>% test_roc(data = data_test) %>% auc()
  ...
  results$AUC[i]<-ad_rf %>% test_roc(data = data_test) %>% auc()
  
  #tree_num <- which(ad_rf$finalModel$forest$ndbigtree == median(ad_rf$finalModel$forest$ndbigtree))[1]
  #tree_func(final_model = ad_rf$finalModel, tree_num)
}
...
tree_func <- function(final_model =  ad_rf$finalModel, 
                      tree_num) {
  
  # get tree by index
  tree <- randomForest::getTree(ad_rf$finalModel, 
                                k = tree_num, 
                                labelVar = TRUE) %>%
...
test_roc <- function(model, data) {
  roc(data$Completers, predict(ad_rf, data, type = "prob")[, "Completers"])
}
